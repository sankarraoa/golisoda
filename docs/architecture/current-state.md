...
| `heritage_immersive` | `0014_tpl_heritage` (insert), `0015_heritage_pres` (presentation/notes refresh) | `public-feedback.css` + `public-feedback-heritage.css` (`.public-shell[data-template="heritage_immersive"]`) | Single-page; progress `none`; touch `large_targets=true`; NPS numeric; CSAT renderers: `emoji_5/emoji_4/emoji_2` | Uses `survey.description` as ornamental footer tagline in `FeedbackFlow` |
